...
RSpec.describe Users::BatchActivate do
  ...
  describe 'execute!' do
    it 'invokes user activation' do
      expect(Users::Activate).to receive(:new)
        .twice
        .and_return(double(Users::Activate, execute!: true, success?: true))
      subject.execute!
    end

    it 'returns successfully sent invites count' do
      stub_service(Users::Activate)
      subject.execute!
    ...
    it 'returns failed invitations count' do
      stub_service(Users::Activate, false, user: instance_double(User, active?: false))
      subject.execute!
    ...
    it 'returns skipped invitations count' do
      stub_service(Users::Activate, false, user: instance_double(User, active?: true))
      subject.execute!
      expect(subject.succeeded_ids).to eq([])
      expect(subject.failed_ids).to eq([])
