...
class AvatarCropper extends Component {
  ...
  render() {
    const { active, closeAvatarCropper, loading, image, handleCrop } = this.props
    const { apply } = this.state

    return (
      <Dialog onClose={ closeAvatarCropper } active={ active } width={ 900 }>
        <DialogHeader close>
          Adjust Image
        </DialogHeader>
        <DialogBodyStyled>
        ...
        <DialogFooterStyled>
          {
            loading ? <Loader color="#FDB924" />
              : <Fragment>
                ...
                <Cancel onClick={ closeAvatarCropper }>Cancel</Cancel>
              </Fragment>
          }
        </DialogFooterStyled>
      </Dialog>
