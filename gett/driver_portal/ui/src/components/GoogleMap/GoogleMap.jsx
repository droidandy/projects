...
import { isEqual, isEmpty, flatMap, compact } from 'lodash'
...
export default class GoogleMap extends PureComponent {
  ...
  fitToCenter(props, { auto, center } = {}) {
    if (center) {
      ...
      this.getAllCoordinates(props).forEach(m => {
        if (!isEmpty(m) && m.lat && m.lng) {
          bounds.extend(toLatLng(this.getLatLng(m)))
        }
      })
  ...
  render() {
    ...
    return (
      <div ref={ div => this.domNode = div } className={ className } style={ { width, height, ...style } }>
