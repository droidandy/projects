...
feature 'Back Office Bookings - Edit Order' do
  ...
  scenario 'International Order' do
    ...
    expect(edit_booking_page).to be_displayed
    ...
    expect(order.journey.pickup_address.text).to include('48 Rue Ernest Renan')
    expect(order.journey.destination_address.text).to include('221B Baker St')
  end
  ...
  feature 'Future Orders' do
    ...
    scenario 'POI as PickUp and Destination Addresses' do
      ...
      expect(order.journey.pickup_address.text).to eql(PredefinedAddress.first.line)
      expect(order.journey.destination_address.text).to eql(PredefinedAddress.last.line)
    end
    ...
    scenario 'Add, remove and change Stop Points' do
      ...
      edit_booking_page.submit
      expect(edit_booking_page.stop_points.second.name.error_message).to eql("can't be blank")
      expect(edit_booking_page.stop_points.second.phone.error_message).to eql("Please add in the passenger's phone number")
      expect(edit_booking_page.stop_points.second.address.error_message).to eql("Please add in the pickup.")
    ...
    [:ot, :gett, :ru, :il, :get_e, :carey, :manual].each do |vendor|
      context "for #{vendor} order" do
        let(:pick_addr) { create(:address, pickup) }
        let(:dest_addr) { create(:address, destin) }
        let(:pickup)    { :baker_street }
        let(:destin)    { :mercedes_glasgow }

        case vendor
        when :ru
          let(:pickup) { :red_square }
          let(:destin) { :mgu }
          let(:car)    { :ot }
        when :il
          let(:pickup) { :shalom_hotel }
          let(:destin) { :haifa_airport }
          let(:car)    { :ot }
        when :ot, :gett, :get_e, :manual
          let(:car)    { vendor }
        when :carey
          let(:car)    { :carey }
          let(:pickup) { :new_york }
          let(:destin) { :new_york_1 }
        end

        let(:booking) do
          create(:booking, :scheduled, car,
            pickup_address: pick_addr,
            destination_address: dest_addr,
            passenger: passenger,
            booker: company.admin,
            scheduled_at: 121.minutes.from_now)
        end
        ...
        scenario 'allow to be edited' do
          login_as_super_admin
          edit_booking_page.load(order: booking.id)
          edit_booking_page.message_to_driver.set 'Hello there!'
          edit_booking_page.submit

          expect(wait_for{ bookings_page.bookings.first }.details.message_to_driver.text).to eql('Hello there!')
        end
      end
    end
  end
