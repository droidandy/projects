...
RSpec.describe InvoicePayments::CreateAutomatic, type: :service do
  let(:company) { create(:company) }

  subject(:service) { described_class.new(company: company) }

  before do
    create(:payment_card, passenger: nil, company: company, token: 'card_token')
    invoice_ids = create_list(:invoice, 2, amount_cents: 1000, company: company).map(&:id)
