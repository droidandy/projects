...
RSpec.describe Bookings::ChargesUpdaters::GetE, type: :service do
  let(:address) { create(:address) }
  let(:booking) { create(:booking, :get_e, :without_passenger, pickup_address: address, fare_quote: 10000) }
  ...
  describe '#execute' do
    ...
    context 'when booking was created by reincarnated admin' do
      ...
      let(:booking) do
        create(:booking, :reincarnated, :get_e, :without_passenger, fare_quote: 10000, booker: admin)
      end
    ...
    context 'when booking is not vatable' do
      ...
      let(:booking) { create(:booking, :get_e, :without_passenger, fare_quote: 10000, company: company) }
