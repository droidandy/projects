...
class Feedback extends Component {
  state = {
    currentOption: {
      title: "Why did you cancel a subscription?",
      id: 0
    }
  };
  ...
  render() {
    ...
    const { appId, screen } = this.props
    const { feedbacks_count, presentations_count, answers, question } = screen
    ...
    return (
      <div className="dashboard-group__content">
        <div className="dashboard-row">
          <div className="rules-analyze__box">
            <DashboardItem
              ...
              tipDescription="Number of times feedback screen was shown to users."
              tipButtonUrl=""
              loading={false}
            />
          </div>
          <div className="rules-analyze__box">
            <DashboardItem
              ...
              autorenews={false}
              tipTitle="Feedbacks"
              tipDescription="Number of responses received."
              tipButtonUrl=""
              loading={false}
            />
          </div>
        ...
        <div className="rules-analyze__answers">
          {answers.map((answer, index) => (
            <div className="rules-analyze__answers-item" key={index}>
              <NavLink
                target="_blank"
                to={`/apps/${appId}/users/${answer.customer_id}`}
                className="link link_normal"
              >
                {answer.customer_id}
              </NavLink>
              <div className="rules-analyze__answers-item__text">
                {answer.text}
              </div>
              <div className="rules-analyze__answers-item__footer">
                <div className="rules-analyze__answers-item__footer-item">
                  <Moment
                    className="uppercase"
                    format="MMM DD, Y"
                ...
                {/* <div className="rules-analyze__answers-item__footer-item">
                  APP VERSION: {answer.version}
                </div> */}
              </div>
            </div>
          ))}
          {answers.length === 0 && (
            <div className="rules-analyze__answers-item">
              <div className="rules-analyze__answers-item__empty ta-center">
