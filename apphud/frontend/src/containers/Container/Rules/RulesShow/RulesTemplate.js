...
import IPhoneVideo from "../../../../containers/Common/IPhoneVideo"

class RulesTemplate extends Component {
  state = {
    showVideo: false
  };
  ...
  render() {
    ...
    return (
      <div className="rules-templates__item">
        ...
        {tutorialUrl && (
          <div className="tip" onClick={this.handleOpenVideo}>
        ...
        {showVideo && (
          <IPhoneVideo close={this.handleCloseVideo} videoUrl={tutorialUrl} />
        )}
        <div className="rules-templates__item-icon">
          <img src={icon} alt="Win back" />
        </div>
        <h2 className="rules-templates__item-title">{title}</h2>
        <div className="rules-templates__item-description">{description}</div>
        <button
          disabled={loading}
          onClick={buttonAction}
          className="button button_green button_stretch rules-templates__item-button"
        >
...
export default connect(mapStateToProps, mapDispatchToProps)(RulesTemplate)
