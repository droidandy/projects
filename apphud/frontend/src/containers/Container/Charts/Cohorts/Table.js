...
import Checkbox from "../Charts/Tables/Checkbox";
import CustomSelect from "../../../Common/CustomSelect";
import SortableButton from "../Charts/DatasetsFilter/SortableButton";
import Toggle from "../Charts/DatasetsFilter/Toggle";
import Tip from "../../../Common/Tip";
import ChartDataTransformerWithNestedRows from "../Charts/transformers/ChartDataTransformerWithNestedRows";
import {getValue} from "../Charts/utils/renderValue";
...
const Table = ({ id = "", data, disabledData, onDisableHandler, onAllDisableHandler, onOpenExport, segment, sortable, defaultSort = false, tooltips = {}, isPercentValuesHandler }) => {
    const views = [
    ...
    const filterValues = (e) => {
        return  id === "revenue_retention" ? e.name.toLowerCase() !== "average" : true
    }
    return (
                ...
                <table className='block-table'>
                    <tbody>
                        <tr className='tr'>
                            <th className='th'>
                                { segment
                                    && <Toggle
                                        ...
                                        <SortableButton value={Number(sort?.name === "segment")} onChange={(v) => setSort({order: v, name: "segment"})}>
                            ...
                            { data[0].values.filter(filterValues).map((e, i) => (
                            ...
                            return (
                                <>
                                    <tr className="tr" key={i}>
                                        ...
                                        { item.values.filter(filterValues).map((e, b) => (
                                            <CellRender
                                                id={id}
                                                rootValue={rootValue}
                                                rootIndex={rootIndex}
