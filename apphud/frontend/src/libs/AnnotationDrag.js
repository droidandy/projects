...
function AnnotationDrag(args) {
  ...
  this.onDragStart = (event) => {
    ...
    if (isDraggable) {
      ...
      this.fakeGhost = event.target.cloneNode(true)
      this.fakeGhost.style.opacity = 0
      document.body.appendChild(this.fakeGhost)
      dt.setDragImage(this.fakeGhost, 0, 0)
    }
  }
  ...
  this.onDragEnd = (event) => {
    ...
    if (this.fakeGhost) this.fakeGhost.remove()
  }
  ...
  this.getRangeAfterRange = (sourceRange, length) => {
    ...
    if (newEnd > sourceRange.endContainer.length) {
