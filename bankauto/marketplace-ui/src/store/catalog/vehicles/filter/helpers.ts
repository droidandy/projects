...
function getValuesToFlush(data: VehiclesFilterData, values: VehiclesFilterValues): Partial<VehiclesFilterValues> {
  ...
  const TO_PART = 'To';
  const arrayFields = ['brands', 'models', 'generations', 'bodyTypes', 'transmissions', 'engines', 'drives'] as const;
...
function getUpdatedPrices(
  ...
  previousData: VehiclesFilterData,
): Partial<VehiclesFilterValues> {
  const updatedPrices: Partial<VehiclesFilterValues> = {};
  const prices = ['priceFrom', 'priceTo'] as const;

  Object.keys(data).forEach((value) => {
    if (values && prices.includes(value as typeof prices[number])) {
      const field = value as typeof prices[number];
      if (values[field]) {
        updatedPrices[field] = `${
          previousData[field] === +(values[field] as string) ? data[field] : +(values[field] as string)
        }`;
      }
    }
  });
...
export { getValuesToFlush, getUpdatedPrices };
