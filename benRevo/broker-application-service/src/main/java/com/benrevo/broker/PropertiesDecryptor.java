package com.benrevo.broker;

import static com.amazonaws.regions.Regions.US_EAST_1;
import static com.amazonaws.regions.Regions.US_WEST_2;
import static com.benrevo.common.enums.CarrierType.fromString;
import static com.benrevo.broker.PropertiesDecryptor.PropertyResource.Key.auth0;
import static com.benrevo.broker.PropertiesDecryptor.PropertyResource.Key.aws;
...
import com.benrevo.broker.PropertiesDecryptor.PropertyResource.Key;
...
public class PropertiesDecryptor implements EnvironmentPostProcessor, Ordered {
    ...
    static final String DIR = "%s";

    // Property resources defined in order of precedence
    enum PropertyResource {

        // Auth0
        auth0_override(auth0, US_WEST_2, false, false),
        auth0_preprod(auth0, US_WEST_2, false, true),
        auth0_prod(auth0, US_WEST_2, true, true),

        // AWS
        aws_override(aws, US_WEST_2, false, false),
        aws_preprod(aws, US_WEST_2, false, true),
    ...
    public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication app) {
        ...
        // Initialize kms
        if(kms == null) {
            kms = AWSKMSClient.builder().withRegion(getRegion(env)).build();
        }

        // Determine whether to use the preprod or prod properties
        boolean prod = equalsIgnoreCase(APP_ENV, "prod");

        // Attempt to load each property source
        for(PropertyResource pr : PropertyResource.values()) {
            if(!ps.contains(pr.key.name()) && pr.prod == prod) {
    ...
    Regions getRegion(ConfigurableEnvironment env) {
        // TODO: This should just return the APP_REGION property.
        return equalsIgnoreCase(APP_ENV, "prod") ? US_WEST_2 : US_WEST_2;
    }
}
