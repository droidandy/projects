...
import PlanRxDropdown from './../components/PlanRxDropdown';
export const finalQuoteState = QuoteState.mergeDeep(additionalState);

configure({ adapter: new Adapter() });

describe('<PlanRxDropdown />', () => {
  const middlewares = [];
  ...
  it('should render the PlanRxDropdown', () => {
    ...
    expect(renderedComponent.find('.plan-rx-dropdown').length).toBe(2);
  });
});
