...
import PlanNetworksDropdown from './../components/PlanNetworksDropdown';
export const finalQuoteState = QuoteState.mergeDeep(additionalState);

configure({ adapter: new Adapter() });

describe('<PlanNetworksDropdown />', () => {
  const middlewares = [];
  ...
  it('should render the PlanNetworksDropdown', () => {
    const renderedComponent = mount(
      <Provider store={store}>
        <IntlProvider locale="en">
          <PlanNetworksDropdown
            section={section}
    ...
    expect(renderedComponent.find('.plan-networks-dropdown').length).toBe(2);
  });
});
