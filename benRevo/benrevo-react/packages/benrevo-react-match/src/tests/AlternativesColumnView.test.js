...
import AlternativesColumnView from './../components/AlternativesColumnView';
...
describe('<AlternativesColumnView />', () => {
  const middlewares = [];
  ...
  it('should render the AlternativesColumnView', () => {
    const renderedComponent = mount(
      <Provider store={store}>
        <IntlProvider locale="en">
          <AlternativesColumnView
            section={section}
    ...
    expect(renderedComponent.find('.alternatives-table-column.current.current-empty').length).toBe(1);
  });
  ...
  it('should render the AlternativesColumnView', () => {
    ...
    expect(renderedComponent.find('.alternatives-block').length).toBe(1);
    expect(renderedComponent.find('.alternatives-table-column.current.current-empty').length).toBe(2);
  });

  it('should render the AlternativesColumnView with rx columns', () => {
    const renderedComponent = mount(
      <Provider store={store}>
        <IntlProvider locale="en">
          <AlternativesColumnView
    ...
    expect(renderedComponent.find('.alternatives-block').length).toBe(1);
    expect(renderedComponent.find('.rx-column.current-empty').length).toBe(1);
    expect(renderedComponent.find('.rx-column').length).toBe(3);
  });
});
