...
export function carriersGetSuccess(state, action) {
  ...
  action.payload.map((item) => {
    if (CARRIER === 'UHC' && item.carrier.name === 'UHC') {
      mainCarrier = Map(item);
    } else if (CARRIER === 'ANTHEM' && item.carrier.name === 'ANTHEM_BLUE_CROSS') {
      mainCarrier = Map(item);
    } else if (CARRIER === 'ANTHEM' && item.carrier.name === 'ANTHEM_CLEAR_VALUE') {
