...
import ClientsSagas from './pages/Clients/sagas';
import UploadQuoteSagas from './pages/Prequote/Quote/sagas';
...
export default function configureStore(initialState = {}, history, callback, persist) {
  ...
  sagaMiddleware.run(ClientsSagas[0]);
  sagaMiddleware.run(watchSaveClient);
  sagaMiddleware.run(watchFetchData);
  sagaMiddleware.run(UploadQuoteSagas[0]);
