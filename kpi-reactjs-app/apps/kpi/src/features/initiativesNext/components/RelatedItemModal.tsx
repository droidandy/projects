...
import { Initiative, BalancedScorecard, RelatedItem } from 'src/types-next';
import {
  getAllExpandedMap,
  getScorecardTree,
  getInitiativeTree,
...
export function RelatedItemModal() {
  ...
  return (
    <SidePanel isOpen={isVisible} title={t('Add Related Item')} close={close}>
      {renderDetails()}
    </SidePanel>
  );
}
...
export const [handle, RelatedItemActions, getRelatedItemState] = createModule(
  ...
  .withActions({
    ...
    itemCreated: (item: RelatedItem) => ({ payload: { item } }),
    expandAll: null,
  })
  ...
  .on(RelatedItemFormActions.setSubmitSucceeded, () => {
    ...
    if (isEditing) {
      const item = {
        ...
        toObjectTitle: treeItems.find(x => x.key === selected)!.name,
      } as RelatedItem;

      return [RelatedItemActions.itemCreated(item), RelatedItemActions.close()];
    } else {
      const { initiative } = getInitiativesState();
      const item = {
        fromObjectType: 'InitiativeItem',
        fromObjectId: initiative!.id,
        toObjectType: toType,
        toObjectId: Number(toId),
      } as RelatedItem;
      return Rx.concatObs(
        Rx.of(RelatedItemActions.setSaving(true)),
        createRelatedItem(item).pipe(
          Rx.mergeMap(created => [
            RelatedItemActions.itemCreated(created),
