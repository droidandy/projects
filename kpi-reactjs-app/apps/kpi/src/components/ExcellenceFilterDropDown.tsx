...
import {
  ExcellenceActions,
  getExcellenceState,
} from '../features/excellence/interface';
...
export function ExcellenceFilterDropDown() {
  const { updateFilter, applyFilter, clearFilter } = useActions(
    ExcellenceActions
  );
  const { filter } = getExcellenceState.useState();
  ...
  const value =
    filter.responsibleUnitId && organizationUnits
      ? organizationUnits.find(x => x.id === filter.responsibleUnitId)
      : defaultOptionValue;
      ...
      <MenuDropdown
        ...
        dropdown={
          <DropdownWrapper>
            <Tree
              selected={
                filter.responsibleUnitId ? filter.responsibleUnitId : 'all'
              }
              onChange={x =>
                x.id === 'all'
                  ? clearFilter() && applyFilter()
                  : updateFilter('responsibleUnitId', x.id) && applyFilter()
              }
