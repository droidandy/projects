...
class _ProductListItemsState extends State<ProductListLayout> with AfterLayoutMixin {
  ...
  Future<List<Product>> getProductLayout(context) {
    if (widget.config["layout"] == "recentView")
      return Provider.of<RecentModel>(context, listen: false).getRecentProduct();
                ...
                switch (snapshot.connectionState) {
                  ...
                  case ConnectionState.waiting:
                    return Column(
                      children: <Widget>[
                        HeaderView(
                          headerText: widget.config["name"] != null ? widget.config["name"] : '',
                          showSeeAll: isRecent ? false : true,
                          callback: () => Product.showList(context: context, config: widget.config),
                        ),
                        ConstrainedBox(
                          constraints: BoxConstraints(
                            minHeight: _buildProductHeight(constraint.maxWidth, isTablet) ?? 0,
                          ),
                          child: SingleChildScrollView(
                            scrollDirection: Axis.horizontal,
                            child: Row(
                      ...
                      return Column(
                        children: <Widget>[
                          ...
                          widget.config["layout"] == "staggered"
                              ? ProductStaggered(snapshot.data, constraint.maxWidth)
                              : getProductListWidgets(snapshot.data, constraint.maxWidth)
                        ],
                      );
                    }
                }
