...
class PaypalServices{
  String domain = PaypalConfig["production"] == true ? "https://api.paypal.com" : "https://api.sandbox.paypal.com";
  ...
  Future<Map<String, String>> createPaypalPayment(transactions, accessToken)async{
    try {
      var response = await http.post(
          ...
          body:
          convert.jsonEncode(transactions),
  ...
  Future<String> executePayment(url, payerId, accessToken)async{
    try {
      var response = await http.post(
          url,
          body:
          convert.jsonEncode({"payer_id":payerId}),
          headers: {"content-type": "application/json", 'Authorization': 'Bearer ' + accessToken});
