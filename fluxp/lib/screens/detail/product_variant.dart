...
class StateProductVariant extends State<ProductVariant> {
  ...
  Future<List<ProductVariation>> getProductVariantions() async {
    ...
    if (variations.isEmpty) {
      ...
      services.getProduct(product.id).then((onValue) {
        if (onValue != null) {
          setState(() {
            product = onValue;
          });
    ...
    for (var variant in variations) {
      if (variant.attributes.length == mapAttribute.keys.toList().length) {
        ...
        for (var i = 0; i < variant.attributes.length; i++) {
          if (variant.attributes[i].option != mapAttribute[variant.attributes[i].name]) {
            check = false;
            break;
          }
        }
  ...
  void checkVariation() {
    if (variations != null) {
      ...
      if (variation == null && variations.length > 0) variation = variations[0];
      if (variation != null) {
        setState(() {
          productVariation = variation;
        });
        Provider.of<ProductModel>(context, listen: false).changeProductVariation(variation);
      }
    }
  }
  ...
  List<Widget> getProductVariant() {
    ...
    final checkProductAttribute = product.attributes != null && product.attributes.isNotEmpty;
    bool inStock = productVariation != null ? productVariation.inStock : product.inStock;
    ...
    if (checkProductAttribute) {
      for (var attr in product.attributes) {
        if (attr.name != null && attr.name.isNotEmpty) {
          ...
          String selectedValue = mapAttribute[attr.name] != null ? mapAttribute[attr.name] : "";
          if (serverConfig["type"] == "magento") {
            final o =
                attr.options.firstWhere((f) => f["value"] == selectedValue, orElse: () => null);
            if (o != null) {
                ...
                if (serverConfig["type"] == "magento") {
                  setState(() {
                    ...
                    mapAttribute.update(attr.name, (value) {
                      final option =
                          attr.options.firstWhere((o) => o["label"] == val, orElse: () => null);
                      if (option != null) {
                        return option["value"];
                      }
                      return val;
