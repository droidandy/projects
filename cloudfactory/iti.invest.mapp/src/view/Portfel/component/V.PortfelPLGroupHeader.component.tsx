...
export class VPortfelPLGroupHeader extends React.Component<IVPortfelPLGroupHeader> {
  ...
  public render() {
    ...
    const account =  model?.accountX.model

    return (
      <VRow justifyContent={'space-between'} alignItems={'center'} {...flexProps}>
        <VCol>
          <VText mb={space.sm}>{`Баланс ${account ? 'счёта' : 'портфеля'}`}</VText>
          <VPortfelPLGroupStat size={'lg'} model={model} />
        </VCol>
        <VButton.Fill color={color.accent1} width={41} height={30} onPress={onCurrency}>
