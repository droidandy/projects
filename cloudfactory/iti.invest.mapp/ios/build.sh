...
react_native_webview_patch() {
   if [ "$BUILD_TARGET" == "preproduction" ]; then
      echo "[INFO] Patch react-native-webview"
     (cd .. && git apply ./react_native_webview_patch/react-native-webview.patch)
   fi
}
