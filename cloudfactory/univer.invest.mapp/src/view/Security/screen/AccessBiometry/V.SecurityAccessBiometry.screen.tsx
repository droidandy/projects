...
import {
  IVSecurityAccessBiometryPresentProps, VSecurityAccessBiometryPresent, VSecurityAccessBiometryPresentTid,
} from '@invest.wl/view/src/Security/present/V.SecurityAccessBiometry.present';
import {
  VButton, VCol, VContainer, VContent, VIcon, VNavBar, VRow, VStatusBar, VStubError, VSwitch, VText,
    ...
    return (
      <VContainer>
        ...
        <VNavBar>
          ...
          <VNavBar.RightText title={'ВЫЙТИ'} onPress={pr.signOut} color={theme.color.primary1} />
        </VNavBar>
        <VContent pa={20} bg={theme.color.bg}>
          {this._contentRender()}
          <VButton.Stroke mt={theme.space.lg} color={theme.color.primary1} onPress={pr.mainGo}>НЕ СЕЙЧАС</VButton.Stroke>
        </VContent>
      </VContainer>
    );
  }
    ...
    return (
      <>
        ...
        <VCol flex justifyContent={'center'} alignItems={'center'}>
          <VIcon name={biometry.iconName} fontSize={72} color={theme.color.primary1} />
        </VCol>
        ...
        <VButton.Fill mt={theme.space.xl} color={theme.color.primary1} onPress={pr.access}>
          ПОДКЛЮЧИТЬ
        </VButton.Fill>
      </>
    );
