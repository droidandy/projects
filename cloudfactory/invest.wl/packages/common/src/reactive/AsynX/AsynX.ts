...
import { AsynXDeep } from './AsynX.deep';
import { AsynXExtendable } from './AsynX.extendable';
import { AsynXPaged } from './AsynX.paged';
import { IAsynX, IAsynXOpts, TAsync, TAsyncArguments, TAsyncReturn } from './AsynX.types';

export class AsynX<Api extends TAsync = TAsync, Req extends TAsyncArguments<Api> = TAsyncArguments<Api>, Res extends TAsyncReturn<Api> = TAsyncReturn<Api>>
  extends AsynXExtendable<Api, Req, Res> implements IAsynX<Api, Req, Res> {
  public static Paged = AsynXPaged;
  public static Deep = AsynXDeep;

  @observable.ref protected _data?: Res;
    ...
    super(_apiFn, _opts, {
      onError: (err) => {
        if (this._opts.retryOnError) {
          ...
          const needRetry = (typeof this._opts.retryOnError === 'number' && this._opts.retryOnError >= this._onErrorRetryCount)
            || this._opts.retryOnError === true;
