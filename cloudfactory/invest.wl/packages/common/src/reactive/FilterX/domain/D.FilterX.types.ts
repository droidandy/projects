...
export type TDFilterXArrayOperator = 'include' | 'exclude';

export type TDFilterXOperator = TDFilterXStringOperator | TDFilterXNumberOperator | TDFilterXDateOperator
| TDFilterXBoolOperator | TDFilterXArrayOperator;

export type TDFilterXOperatorMapper<T extends TDFilterXType> = (T extends Date ? TDFilterXDateOperator
  : T extends number ? TDFilterXNumberOperator : T extends string ? TDFilterXStringOperator : TDFilterXBoolOperator)
| TDFilterXArrayOperator;
...
export interface IDFilterXModel<T extends TDFilterXType, O extends TDFilterXOperator = TDFilterXOperatorMapper<T>>
  extends IDFilterXItem<T>, IDModelXValue<T | undefined> {
  operator?: O;
  clone(): IDFilterXModel<T>;
}

export interface IDFilterXComposeModel<T extends TDFilterXType>
  extends IDFilterXItem<T>, IDModelXValue<IDFilterXModelVariant<T>[]> {
...
export type IDFilterXModelVariant<T extends TDFilterXType, O extends TDFilterXOperator = TDFilterXOperatorMapper<T>> =
  IDFilterXModel<T> | IDFilterXComposeModel<T>;
...
export interface IDFilterXConfigItem<T extends TDFilterXType>
  ...
  list?: ILambda<IDFilterXSelectItem<T>[]>;
}

export interface IDFilterXConfigModel<T extends TDFilterXType>
  extends IDModelXValue<IDFilterXSelectItem<T>[] | undefined>, IDFilterXConfigItemBase {
}
