...
export class NumberX {
  ...
  @observable private _inputLV: ILambda<TNumberXIn>;

  @computed
  public get inputRaw() {
    return lambdaResolve(this._inputLV);
  }

  @computed
  public get value(): number | undefined {
    if (!this._parts || !this.string) return undefined;
    const { int, delimiter } = this._parts;
    if (!int || (NumberX._isIntValidPartial(int) && !delimiter)) return undefined;
  ...
  @computed
  private get _opts() {
    return { ...DEFAULT_OPTS, ...lambdaResolve(this._optsLV) };
  }

  constructor(numStrLV: ILambda<TNumberXIn>, private _optsLV: ILambda<INumberXOpts>) {
    this._inputLV = numStrLV;
    makeObservable(this);
  }

  @action
  public inputSet(inputLV: ILambda<TNumberXIn>) {
