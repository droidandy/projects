...
export class Formatter {
  public static currency(num: number = 0, presetOrOpts: FormatterPreset | CurrencyFormatterOptions = {},
    ...
    if (!opts.code && !opts.symbol) return this.number(num, opts);
    if (opts.priceStep) this._priceStepHandle(opts);

    if (opts.code) opts.code = this._codeCheckWrong(opts.code);
  ...
  public static date(date: Date | moment.Moment | string, opts?: DateFormatOptions): string {
    ...
    if (opts.pattern === 'default') {
      ...
      const dayFormat = elseFormat?.replace(/^[\s.DMY]*/,'');
      result = opts.calendar ? md.calendar(undefined, {
        sameDay: '[Сегодня]' + dayFormat, lastDay: '[Вчера]' + dayFormat,
        sameElse: elseFormat, lastWeek: elseFormat,
      }) : md.format(elseFormat);
    }
    return result;
  ...
  // только необходимое для правильного набора (цифры и +)
  public static phoneSanitize(phone: string) {
