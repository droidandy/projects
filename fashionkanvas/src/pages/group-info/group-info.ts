...
export class GroupInfoPage {
  ...
  ionViewDidLoad() {
    ...
    this.subscription = this.dataProvider.getGroup(this.groupId).snapshotChanges().subscribe((groupRes:any) => {
      let group:any = { $key: groupRes.key, ... groupRes.payload.val()};
      console.log(group);
      if (group != null) {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.imageProvider.setGroupPhotoPromise(this.group, this.camera.PictureSourceType.CAMERA).then((group:any) => {
              // Add system message.
              this.group.messages.push({
                date: new Date().toString(),
                sender: this.user.$key,
  ...
  getUser(){
    ...
    this.dataProvider.getCurrentUser().snapshotChanges().subscribe((account:any) => {
      ...
      this.dataProvider.getAllUser().snapshotChanges().subscribe((account) => {

      //  console.log('account.payload.val().friends===',account.payload.val());
        if(account.payload.val()){
          var tempdiscoverList = account.payload.val();
          this.discoverList =[];
  ...
  openDiscover(item){
    this.navCtrl.push(DiscoverPage, { userId: item.userId });
  }
}
