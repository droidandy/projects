    ...
    this.dataProvider.getConversations().snapshotChanges().subscribe((conversationsInfoRes) => {
      ...
      if (conversations.length > 0) {
        conversations.forEach((conversation) => {
          ...
          if (conversation) {
            conversation.tempblocked =false;
            if(conversation.blocked!=undefined&&conversation.blocked!=null){
              conversation.tempblocked = conversation.blocked;
            }

            // Get conversation partner info.
            this.dataProvider.getUser(conversation.key).snapshotChanges().subscribe((user:any) => {
              conversation.friend = user.payload.val();
              for(let data in conversation.friend.conversations){
                  if(conversation.friend.conversations[data].conversationId==conversation.conversationId&&!conversation.tempblocked){
                    if(conversation.friend.conversations[data].blocked!=undefined&&conversation.friend.conversations[data].blocked!=null){
                      conversation.blocked = conversation.friend.conversations[data].blocked;
                    }
                  }
              }
