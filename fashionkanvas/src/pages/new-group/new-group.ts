...
export class NewGroupPage {
  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getCurrentUser().snapshotChanges().subscribe((accountRes:any) => {
      ...
      this.dataProvider.getAllUser().snapshotChanges().subscribe((account:any) => {

      //  console.log('account.payload.val().friends===',account.payload.val());
        if(account.payload.val()){
          var tempList = account.payload.val();
          for(let data in tempList){

              if(account.userId!=data){

                let friend = { $key: data, ... tempList[data]};
