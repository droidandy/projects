...
export class ImageProvider {
  ...
  private discoverPhotoOptions: CameraOptions = {
    quality: 75,
    targetWidth: 384,
    targetHeight: 384,
    allowEdit:true,
    ...
    this.camera.getPicture(this.profilePhotoOptions).then((imageData) => {
      ...
      firebase.storage().ref().child('images/' + user.userId + '/' + this.generateFilename()).put(imgBlob, metadata).then((snapshot) => {
        ...
        snapshot.ref.getDownloadURL().then((downloadURL) => {
            console.log('File available at', downloadURL,this);
            let url = downloadURL;
            let profile = {
  ...
  setAddPhoto(user, sourceType): Promise<any> {
    console.log('set add photo2==========================');
    return new Promise(resolve => {

      this.discoverPhotoOptions.sourceType = sourceType;
      this.loadingProvider.show();
      this.camera.getPicture(this.discoverPhotoOptions).then((imageData) => {
        // Process the returned imageURI.
        let imgBlob = this.imgURItoBlob("data:image/jpeg;base64," + imageData);
        let metadata = {
          'contentType': imgBlob.type
        ...
        // Generate filename and upload to Firebase Storage.
        console.log(imgBlob);
        console.log('userphoto/' + user.userId + '/' + this.generateFilename());
        firebase.storage().ref().child('userphoto/' + user.userId + '/' + this.generateFilename()).put(imgBlob, metadata).then((uploadTask) => {
          ...
          uploadTask.ref.getDownloadURL().then((downloadURL) => {
              ...
              this.angularfire.object('/accounts/' + user.userId).update({update_at:new Date().getTime()});
              this.angularfire.list('/accounts/' + user.userId+'/photos/').push(imageData).then((success) => {
                this.loadingProvider.hide();
                console.log(success);
                imageData.key = success.key;
                //this.alertProvider.showProfileUpdatedMessage();
                resolve(imageData);
              })
  ...
  setJobPhotoPromise(group, sourceType): Promise<any> {
    return new Promise(resolve => {
      this.groupPhotoOptions.sourceType = sourceType;
      this.loadingProvider.show();
      // Get picture from camera or gallery.
