...
import {
  getCompanyPartnerNetworksAction,
...
const useData = ({ company, allFilters }) => {
  ...
  const { loading: partnerNetworksLoading, payload: partnerNetworks = [] } = useQuery(
    getCompanyPartnerNetworksAction({ cid: company.cid })
  );
  let partnerNetworksCapabilities = [];
  ...
  if (!loading) {
    ...
    partnerNetworksCapabilities = _.union(...partnerNetworks.map(p => p.capabilities)).map(it => {
      const [id, name] = String(it).split(':', 2);
      return { id: +id, name };
    });
  }
...
const BuViewTable = ({ allFilters, company }) => {
  ...
  const { showPartnerNetwork, lense, tags, parentGroupFilter, groupFilter } = allFilters;
  ...
  if (parentGroupFilter && groupFilter) {
    ...
    if (subgroup) {
      colorScale.range(['#094470'].concat(d3.schemePaired)).domain(subgroup.filters);
    }
  } else if (lense !== 'all') {
    const found = lenses.find(l => l.name === lense);
    if (found) {
      colorScale
        .range(['#094470'].concat(d3.schemePaired))
        .domain(found.classifications.map(c => c.name));
    }
  }
  ...
  return loading || (childCaps && childCaps.length === 0) ? (
    ...
    <div className={classNames(classes.buTableContainer, 'table-responsive')}>
      {showPartnerNetwork && <p>Partner Network Capabilities</p>}
            ...
            {parentCaps.map(parent => (
              <td key={parent.id}>
                {parent.children.map(child => {
                  ...
                  if (
                    showPartnerNetwork &&
                    !partnerNetworksCapabilities.map(it => it.id).includes(capability.id)
                  ) {
...
/*
if (showPartnerNetwork) {
    //colorScale.range(['#fdbf6f']).domain(['Customer Engagement']);
  }

  if (showPartnerNetwork) {
                    if (partnerNetworksCapabilities.map(it => it.id).includes(capability.id)) {
                      backgroundColor = colorScale('Customer Engagement');
                    }
                  }
  */
